...
16：サブネット、左から16桁目までが同じネットワーク範囲と指定

VPCは/16 ~ /28のCIDR範囲を使用できる

## サブネット
サブネットはCIDR範囲で分割したネットワークセグメント
サブネットはVPC内の複数設置でき、１つのAZを指定して配置される。パブリックとプライベートがある
インターネットゲートウェイのルーティング有無でサブネットのタイプが分かれる

### パブリックサブネット
トラフィックがインターネえっとゲートウェイにルーティングされるサブネット
### プライベートサブネット
インターネットゲートウェイへのルートがないサブネット
*プライベートネットワークからネットに接続したい場合、NATゲートウェイがパブリックサブネットに必要